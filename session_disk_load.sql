...
begin
 ...
 select inst_id, sid, statistic#, value bulk collect into v_stat from gv$sesstat where statistic# in (select statistic# from gv$statname where name in ('physical read bytes','physical write bytes')) and value != 0;

 dbms_lock.sleep(20);

 for c in (select inst_id, sid, statistic#, value from gv$sesstat where statistic# in (select statistic# from gv$statname where name in ('physical read bytes','physical write bytes')) and value != 0) loop
  is_found := 0;
...
--select * from gv$statname where statistic# in (51,52,55,83,96)
--select statistic# from gv$statname where name in ('physical read bytes','physical write bytes')

--select * from gv$statname where lower(name) like '%bytes%'
--select * from gv$sysstat

--select * from gv$statname where upper(name) like '%NET%'
